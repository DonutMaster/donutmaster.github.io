...
Doesn't seem like it contains much information that we already don't know.

### Login.php
![login page](/assets/img/thm/dreaming/login.png)

Ahhhh, we have a login page! We only need a password and no username. We can use hydra to brute force the login.
...
Let's check around and see any files that seem important.
...
We have to python scripts in the `/opt` directory.
...
We got a mysql password and username, altho it doesn't seem like we can use it for now. Let's check sudo -l.
...
Seems like we can run `/usr/bin/python3` as Death for `/home/death/getDreams.py`
...
It seems like the code is running a command echo with the "dreamer" and the "dream". Let's see if that has to do with mysql possibly.
...
Hmm, none of the commands we execute are not working. Let's try to use mysql to get Death's password from `/home/death/getDreams.py`
