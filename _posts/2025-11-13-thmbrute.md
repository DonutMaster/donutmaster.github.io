...
We got the user flag! Let's check some of the other files on Adrian's home directory.
...
Let's confirm that the `punch_in.sh` file is running every minute by root. It's very important that its root, not adrian, running the script. If it isn't root, we can't escalate our privileges to root. We can use [Pspy](github.com/DominicBreuker/pspy) to check.
